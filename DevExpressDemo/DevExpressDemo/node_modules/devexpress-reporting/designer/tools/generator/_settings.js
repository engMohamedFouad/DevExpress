...
import { patchRequest } from '../../internal/fieldlist/_fieldListDataSourcesHelper';
...
export class WizardsInitializerSettings {
    ...
    createSqlDataSourceEditor(settings) {
        this.federationDataSourceEditor = new FederationDataSourceEditor(settings.dataSourceHelper, settings.dataSourceWizard, settings.model, settings.undoEngine, settings.fieldListProvider, settings.rtl, this.multipleQueriesWizardCallbacks);
        this.sqlDataSourceEditor = new SqlDataSourceEditor(settings.dataSourceHelper, settings.dataSourceWizard, settings.model, settings.undoEngine, settings.fieldListProvider);
    ...
    createMultipleQueriesWizardCallbacks(itemsProvider, model, state) {
        var fieldsCallback = (request, dataSource) => {
            patchRequest(request, dataSource ? [dataSource] : this.multiQueryWizardOptions.dataSources(), state());
            return this.callbacks.fieldLists(request);
        };
