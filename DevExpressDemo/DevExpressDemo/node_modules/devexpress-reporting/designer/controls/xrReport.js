...
export class ReportViewModel extends XRReportElementViewModel {
    constructor(report, serializer, knownEnums) {
        serializer = serializer || new ReportModelSerializer();
        report = report.XtraReportsLayoutSerializer || report;
        super(patchMargins(report), null, serializer);
        ...
        this.parameterHelper = new ReportParameterHelper(this);
        ...
        this._disposables.push(this.paperKind.subscribe((newVal) => {
            if (newVal !== 'Custom') {
                var size = papperKindMapper[newVal];
                this.pageHeight(roundingXDecimals((this.landscape() ? size.width : size.height) * (this._innerDpi.peek() / 100)));
                this.pageWidth(roundingXDecimals((this.landscape() ? size.height : size.width) * (this._innerDpi.peek() / 100)));
            }
        }));
    ...
    enumerateComponents(process = () => void 0) {
        var controls = [];
        collectionsVisitor(this, process, ['controls', 'bands', 'subBands', 'crossBandControls', 'rows', 'cells', 'parameters', 'fields', 'levels'], controls);
    ...
    clone() {
        ...
        var collectStorages = (storage, isComponentStorage = false) => {
            storage.reduce((result, storageItem, index) => {
                var dataSourceRef = this.dataSourceRefs.filter(x => x.ref === storageItem['_model']['@Ref'])[0];
                if (dataSourceRef) {
                    result.push({
                        index,
                        isComponentStorage,
