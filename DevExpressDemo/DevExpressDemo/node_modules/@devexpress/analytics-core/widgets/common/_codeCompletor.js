...
export class CodeCompletor extends Disposable {
    ...
    _getFields(token = null, completions = [], ignorePrefix = false) {
        ...
        this._getRealPath(this._combinePath(parentPrefix)).done((path) => {
            var $fields = ko.unwrap(this._fieldListProvider).getItems(new PathRequest(path))
                .done((fields) => {
                completions.push.apply(completions, this._convertDataMemberInfoToCompletions(CodeCompletor._cleanupFields(fields), token, ignorePrefix ? null : parentPrefix && parentPrefix + '.'));
            });
    ...
    _processFields($deferreds, fields, completions, path, token, parentPrefix = '', needCleanup = false) {
        var pushToCompletions = (fields) => {
            completions.push.apply(completions, this._convertDataMemberInfoToCompletions(needCleanup ? CodeCompletor._cleanupFields(fields) : fields, token, parentPrefix));
        };
        fields.forEach(item => {
